...
export const blogPosts: BlogPost[] = [
  ...
  },
  {
    ...
    title: "Hick's Law: When More Product Options Mean Fewer Customers",
    category: 'Psychology',
    categoryColor: '#854F0B',
    categoryBg: '#FBF3E4',
    date: '05 Jun 2026',
    readTime: '6 min read',
    excerpt: 'A visitor clicks between five color and size options nine times in ten seconds, then scrolls past every bundle choice without stopping. That is not comparison shopping. That is overload.',
    content: [
      { type: 'p', text: 'Product pages exist to make a decision easier. Show the item, lay out the options, and let the shopper pick what fits. In practice, product pages with many variants are one of the most common places where shoppers get stuck.' },
      { type: 'h2', text: 'The pattern: fast, repeated, incomplete' },
      { type: 'p', text: 'Here is a session worth looking at closely. A visitor clicks between the color and size selectors on a product page nine times in about ten seconds, scrolls past three bundle options without pausing, and leaves without adding anything to cart.' },
      { type: 'p', text: 'On the surface, this could look like careful comparison. The visitor is weighing color against size against bundle, doing their homework. But the speed tells a different story. Ten seconds is not long enough to seriously compare nine variant combinations. It is long enough to scan swatches and price deltas and feel that none of it is settling into a decision.' },
      ...
      { type: 'p', text: 'A product page with several color, size, and bundle options, each combination changing price or availability in subtle ways, is a near perfect setup for this. The visitor is not failing to understand the options. They are accurately sensing that comparing all of them fully will take more effort than they are willing to spend right now, on this visit.' },
      { type: 'h2', text: 'Why this looks fine in standard analytics' },
      { type: 'p', text: 'A pageview on a product page looks identical whether the visitor left delighted, confused, or somewhere in between. Even scroll depth on its own is ambiguous, a full scroll could mean thorough reading or a quick once-over. The detail that makes this session readable is the sequence: rapid variant switching, then a fast scroll past the remaining options, then nothing.' },
      { type: 'h2', text: 'What tends to help' },
      { type: 'p', text: 'The direct response to a decision fatigue pattern is to reduce the size of the decision, not the amount of information available overall. That can mean highlighting one variant as the default or most popular, reducing the number of options shown at a glance and linking to detail for the rest, or offering a short, guided way to find the right option based on common use cases rather than asking the shopper to compare every combination themselves.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'p', text: 'Consider a hypothetical example. A test changes a product page\'s call to action from "Add to Cart" to a version that adds a short reassurance line beneath it, something like free returns within 30 days, alongside the original wording. If that variant wins, the explanation does not stop at "the new version converted better." It connects the result back to the mechanism the hypothesis named: reducing perceived risk at the exact moment of commitment lowers the friction of that final click, especially for a shopper who has not bought from this store before.' },
      { type: 'principle', text: 'This connects to a well-established idea from behavioral psychology: risk reversal, where perceived risk is one of the strongest brakes on a purchase decision, especially with an unfamiliar store. Removing that risk right at the point of commitment, rather than earlier on the page, addresses the objection exactly when it is most active in the shopper\'s mind.' },
