...
const CLIENT_PROFILES: Record<string, { name: string; type: string; url: string; industry: string; ctaTargets: string[]; description: string; snippetSince: string }> = {
  ...
  'nexflow': { name: 'Nexflow', type: 'SaaS', url: 'nexflow-demo.vercel.app', industry: 'Project management', snippetSince: '09 Jun 2026', description: 'AI-powered project management tool. Main funnel: landing, pricing, trial signup, workspace setup.', ctaTargets: ['Start free trial', 'Create account', 'Get started', 'Open my workspace'] },
...
export default function DashboardPage() {
  ...
  const fetchTests = useCallback(async () => {
    try {
      ...
      const statsMap: typeof testStats = {}
      await Promise.all(testList.map(async (t) => {
        try {
  ...
  const handleFilter = (key: string) => {
    setFilterKey(key); setDropdownOpen(false); fetchData(key || undefined)
  }
  ...
  const handleSaveToBacklog = async (session: Session) => {
    ...
    try {
      await fetch('/api/backlog', {
        ...
        body: JSON.stringify({
          clientKey: session.client_key, sessionId: session.id,
          insightType: session.insight_type, insightText: session.insight_text,
          recommendation: session.recommendation, estimatedLift: session.estimated_lift,
          state: session.state,
        }),
      })
  ...
  const handleBacklogStatus = async (id: string, status: string) => {
    try {
      ...
      setBacklog(prev => prev.map(i => i.id === id ? { ...i, status } : i))
    } catch (e) { console.error(e) }
  }
  ...
  const handleOpenLaunchModal = (session: Session) => {
    const cta = (session.tags || []).find(t => t.includes('_cta') || t.includes('_button')) || ''
    ...
    try {
      await fetch('/api/tests', {
        ...
        body: JSON.stringify({
          ...
          variantText: launchForm.variantText,
          hypothesis: launchForm.hypothesis,
          sessionSourceId: launchModal.session.id,
          minSessions: 50,
        }),
      })
  ...
  const handleStopTest = async (testId: string) => {
    try {
      await fetch('/api/tests', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: testId, status: 'paused' }) })
      fetchTests()
    } catch (e) { console.error(e) }
  }
  ...
  const handleEvaluateTest = async (testId: string) => {
    try {
      await fetch('/api/tests', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: testId, triggerJudge: true }) })
      fetchTests()
    } catch (e) { console.error(e) }
  }
              ...
              <div className="flex items-start justify-between gap-6 flex-wrap">
                ...
                <div className="flex-1 min-w-[200px]">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {clientProfile.ctaTargets.map(cta => <span key={cta} className="text-[11px] bg-green-light text-green border border-green/20 px-2.5 py-1 rounded-full font-medium">{cta}</span>)}
                  </div>
                </div>
          ...
          <div className="flex gap-1 bg-surface-2 border border-surface-3 rounded-lg p-1 mb-6 w-fit flex-wrap">
            {(['sessions', 'insights', 'backlog', 'tests', 'geo'] as const).map(t => (
              <button key={t} onClick={() => setActiveTab(t)}
                ...
                {t === 'backlog' && backlog.filter(i => i.status === 'pending').length > 0 && (
                  <span className="text-[10px] bg-gold text-white px-1.5 py-0.5 rounded-full">{backlog.filter(i => i.status === 'pending').length}</span>
                )}
                {t === 'tests' && tests.filter(i => i.status === 'active').length > 0 && (
                  <span className="text-[10px] bg-green-light text-green px-1.5 py-0.5 rounded-full">{tests.filter(i => i.status === 'active').length}</span>
                )}
              </button>
            ))}
          </div>
                          ...
                          <div className="grid md:grid-cols-2 gap-4 mt-4">
                            ...
                            <div className="bg-green rounded-lg p-4">
                              ...
                              <div className="flex gap-2">
                                <button onClick={(e) => { e.stopPropagation(); handleOpenLaunchModal(s) }}
                                  className="flex-1 bg-white/15 border border-white/25 text-white py-2 rounded-lg text-[11px] font-mono hover:bg-white/25 transition-colors">
                                  Launch A/B Test
                                </button>
                                <button onClick={(e) => { e.stopPropagation(); handleSaveToBacklog(s) }}
                                  className={`flex-1 border py-2 rounded-lg text-[11px] font-mono transition-colors ${isSaved ? 'bg-[#A8D4B8]/20 border-white/15 text-[#A8D4B8]' : 'bg-white/5 border-white/15 text-white/60 hover:bg-white/10'}`}>
            ...
            <div>
              ...
              {backlog.length === 0 ? (
                <div className="bg-white border border-surface-3 rounded-xl p-12 text-center">
                  ...
                  <div className="text-[13px] text-ink-3">Open a session and click "Save to Backlog" to add recommendations here.</div>
                </div>
              ) : (
                    ...
                    <div key={item.id} className="bg-white border border-surface-3 rounded-xl p-5">
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        ...
                        <div className="flex flex-col gap-2 flex-shrink-0">
                          {item.status === 'pending' && (
                            <button onClick={() => handleBacklogStatus(item.id, 'in_progress')} className="text-[11px] font-mono bg-blue-50 text-blue-700 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition-colors">Start</button>
                          )}
                          {item.status === 'in_progress' && (
                            <button onClick={() => handleBacklogStatus(item.id, 'done')} className="text-[11px] font-mono bg-green-light text-green px-3 py-1.5 rounded-lg hover:opacity-90 transition-opacity">Mark done</button>
                          )}
            ...
            <div>
              ...
              {tests.length === 0 ? (
                <div className="bg-white border border-surface-3 rounded-xl p-12 text-center">
                  ...
                  <div className="text-[13px] text-ink-3">Open a session in the Sessions tab and click "Launch A/B Test".</div>
                </div>
              ) : (
                <div className="space-y-4">
                  {tests.map(test => {
                    ...
                    const bIsWinning = parseFloat(bRate) > parseFloat(aRate)
                    return (
                      <div key={test.id} className={`bg-white border rounded-xl p-5 ${test.status === 'active' ? 'border-blue-200' : test.status === 'completed' ? 'border-green' : 'border-surface-3'}`}>
                        <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
                          ...
                          <div className="flex gap-2">
                            ...
                            {test.status === 'active' && (
                              <button onClick={() => handleStopTest(test.id)} className="text-[11px] font-mono bg-surface-2 text-ink-2 px-3 py-1.5 rounded-lg hover:bg-surface-3">Pause</button>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-4">
                          {(['A', 'B'] as const).map(v => {
                            ...
                            const isLeading = v === 'B' ? bIsWinning : !bIsWinning
                            return (
                              <div key={v} className={`rounded-lg p-4 border ${isWinner ? 'border-green bg-green-light' : 'border-surface-3 bg-surface-2'}`}>
                                ...
                                <div className="text-[13px] font-medium text-ink mb-2">
                                  {v === 'A' ? test.control_text : test.variant_text}
                                </div>
                                {vStats ? (
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={launchForm.elementFindText} onChange={e => setLaunchForm(f => ({ ...f, elementFindText: e.target.value }))}
                  placeholder='e.g. "Buy Now" or "Start free trial"'
                  className="w-full border border-surface-3 rounded-lg px-3 py-2.5 text-[13px] outline-none focus:border-green transition-colors" />
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={launchForm.controlText} onChange={e => setLaunchForm(f => ({ ...f, controlText: e.target.value }))}
                    placeholder="Original text (auto-fill above)"
                    className="w-full border border-surface-3 rounded-lg px-3 py-2.5 text-[13px] outline-none focus:border-green transition-colors" />
                </div>
                <div>
                  <label className="text-[12px] text-ink-2 block mb-1">Variant B</label>
                  <input value={launchForm.variantText} onChange={e => setLaunchForm(f => ({ ...f, variantText: e.target.value }))}
                    placeholder='e.g. "Only 3 left - Buy Now"'
                    className="w-full border border-surface-3 rounded-lg px-3 py-2.5 text-[13px] outline-none focus:border-green transition-colors" />
                </div>
              </div>
              <div>
                <label className="text-[12px] text-ink-2 block mb-1">Hypothesis</label>
                <textarea value={launchForm.hypothesis} onChange={e => setLaunchForm(f => ({ ...f, hypothesis: e.target.value }))} rows={2}
                  className="w-full border border-surface-3 rounded-lg px-3 py-2.5 text-[13px] outline-none focus:border-green transition-colors resize-none" />
              </div>
            </div>

            <div className="bg-surface-2 rounded-lg p-3 mt-4 text-[11px] text-ink-3">
              Traffic split: 50% see Control, 50% see Variant B. Test auto-evaluates after 100 sessions (50 per variant). Judge LLM will explain WHY the winner won.
            </div>
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={handleLaunchTest} disabled={!launchForm.elementFindText || !launchForm.variantText || launching}
                className="flex-1 bg-green text-white py-2.5 rounded-lg text-[13px] font-semibold hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed">
