// Shared prompt source for behavioral session analysis. Used by both
// app/api/analyze/route.ts (production) and eval/run.ts (offline eval), so
// they can never drift apart. Text below is moved verbatim from the route,
// not reworded.

export const MODEL = 'claude-sonnet-4-6'
export const MAX_TOKENS = 1000

// Canonical insight_type enum — single source of truth, used by the API
// route (normalizes AI output before DB write), the eval harness (schema
// adherence scoring), and implicitly the dashboard (which just displays
// whatever the route already normalized).
...
// Maps AI output onto the canonical enum: fixes spacing/casing drift
// ("DECISION FATIGUE" -> "DECISION_FATIGUE") so a formatting slip never
// reaches the DB as an off-enum value. Falls back to a safe sentinel for
// anything unrecognized rather than writing garbage.
export function normalizeInsightType(raw: string | undefined | null): string {
  if (!raw) return 'UNKNOWN'
  const normalized = raw.trim().toUpperCase().replace(/\s+/g, '_')
  return (VALID_INSIGHT_TYPES as readonly string[]).includes(normalized) ? normalized : 'UNKNOWN'
}

// Sessions below this many events don't carry enough signal for a reliable
// AI read (see eval/results — Test 1 groundedness). Enforced server-side in
// the API route, not the tracking snippet, since the route is the one place
// this can be trusted regardless of what the client sends.
export const MIN_EVENTS_FOR_ANALYSIS = 3
...
export function buildUserPrompt(session: AnalyzeSessionInput): string {
  const { clientKey, siteUrl, pageContext, events, sessionDuration, scrollDepth, referral } = session

  return `Analyze this session:
Client: ${clientKey}
